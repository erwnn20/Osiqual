...
<div id="{{ $name .'-container' }}" {{ $attributes->class(['flex', 'flex-col', 'gap-1']) }}>
    ...
    @if($label || $required)
        <label for="{{ $name }}"
               class="w-fit text-sm font-medium text-default-700">
            {{ $label }}
            @if($required)
    ...
    <div class="relative">
        @if($warning)
            <div class="absolute inset-y-0 mx-3 my-auto flex items-center h-fit"
        ...
        <input type="{{ $type }}"
               ...
               class="w-full rounded-lg py-2 px-3 @if($warning) ps-10 @endif
